    ...
    <article className="max-w-2xl mx-auto px-6 py-12 sm:py-20 prose prose-gray prose-lg">
      ...
      <div className="not-prose mt-6 mb-8 flex gap-6 items-start">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 mb-0">Original: {language === "es" ? "Ruso" : "Russian"}</p>
          <p className="text-gray-500 text-sm mt-1 mb-0">{t.blog.edition}: {language === "es" ? "Traducción al inglés" : "English Translation"}</p>
        </div>
      </div>

      {language === "es" ? (
        <p>Noches Blancas no fue un libro fácil de leer. Me encontré leyendo los mismos pasajes una y otra vez para intentar encontrar sentido en la perspectiva del narrador (que en este caso se le refiere como 'el soñador') ya que su nombre nunca es revelado. De cualquier manera, el nombre es irrelevante ya que no tiene nada que ver con el conflicto interno del soñador, ni la corta novela. También reconozco que esta obra fue escrita en ruso originalmente, estoy seguro que las traducciones, aunque comendables, jamás podrán hacerle justicia a la obra original. Quizá eso es evidente en la cadencia de las interacciones interpersonales comparado a lo que yo conozco en español e inglés. Ya que avancé más en la novela, empecé a pensar que este ritmo de conversación al que me refiero, quizá no tenga que ver con lenguaje y cultura, sino con los cambios violentos emocionales y físicos que el soñador y Nastenka viven en encuentros muy cortos pero intensos. Es decir, cuando la emoción trasciende a lo mental y se manifiesta físicamente. Objetivamente, no pasa mucho en esta historia. El núcleo de la historia está más relacionado con qué tan significantes pueden ser los más pequeños momentos, para los que piensan / sienten que la vida está pasando para todos menos ellos. Aún no creo que termine de entender lo que quería comunicar Dostoevsky y mi intención es leerlo una vez más pero esta vez en español. Para explorar las diferencias de esa traducción</p>
      ) : (
